                    ...
                    if (state.encryptActive) {
                        data = JSON.parse(
                            localStorage.getItem(
                                'cache_' + mod + '_' + MD5(lista.url).toString()
                            )
                        )
                        if (data.response != '') {
                        ...
                        data = JSON.parse(
                            localStorage.getItem('cache_' + mod + '_' + lista.url)
                        )
                        if (data) {
                            data = data.response
                        } else {
...
export const actions = {
    logger({ commit, getters, dispatch }, datos) {
        datos = {...datos, app_id: 2, _noData: 1 }
    ...
    async loadDatas({ commit, getters, dispatch }, options) {
        ...
        let response = {}
        try {
            response = await this.$axios.post(options.mod + '/listData', {
                lista: listado
            })
        } catch (error) {
            console.log('error axios', error)
                //response.data = { data: '_ct_', ok: 1 }
            if (error.message === 'Network Error') {
                response.data = { ok: 1, data: {} }
                options.listas.forEach((lista) => {
                    response.data.data[lista.mod] = '_ct_'
                })
                console.log('se recupero del cache')
            }
        }
